...
export const Receive: React.FC<ChildProps> = ({
  ...
  const [EphemeralKeysArray, setEphemeralKeysArray] = useState<any>([]);
  ...
  const getKeys = async () => {
    ...
    setEphemeralKeysArray(await contract.retrievePubKeys(BigNumber.from(initValue)));
    ...
    EphemeralKeysArray.forEach((EphemeralKeys: any) => {

      generateprivatekey()

      try {
        keys = `${EphemeralKeys.sharedSecret.replace("0x", "")}04${EphemeralKeys.x_cor.slice(2)}${EphemeralKeys.y_cor.slice(2)}`;
