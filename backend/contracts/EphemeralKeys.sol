...
// Import the interfaces for IBEP20 and IBEP721 tokens
 ...
 * Users can transfer native coins, BEP20, and Non-fungible tokens to a designated recipient stealth address,
 * authorized by their published keys.
 */

contract EphemeralKeys {
    ...
    // @notice Function to transfer BEP20 tokens to a target stealth address
    // @param x_cor & y_cor: 32-byte ephemeral key

    // @param sharedSecret: 2-byte of stealth address prefixed with ephemeral key
    // @param token: The BEP20 token contract address
    // @param target: The target address (i.e., the recipient's stealth address)
    ...
    function TransferBEP20(
        ...
        require(
            IBEP20(token).balanceOf(msg.sender) >= amount,
            "Not enough tokens"
        );

        if (IBEP20(token).allowance(msg.sender, address(this)) < amount) {
        ...
        IBEP20(token).transferFrom(msg.sender, target, amount);
    ...
    // @notice Function to transfer BEP721 token  to a target stealth address
    // @param x_cor & y_cor: 32-byte ephemeral key

    // @param sharedSecret: 2-byte of stealth address prefixed with ephemeral key
    // @param BEP721Token: The BEP721 token address
    // @param target: The targeted stealth address
    // @param tokenId: The tokenId of BEP721 to transfer

    function TransferBEP721(
        bytes32 x_cor,
        bytes32 y_cor,
        bytes2 sharedSecret,
        address BEP721Token,
        ...
        // Check that BEP721Token is not empty.

        require(BEP721Token != address(0x0), " Enter the token address");

        require(
            IBEP721(BEP721Token).ownerOf(tokenId) == msg.sender,
            "You are not the owner of this tokenId"
        );

        // check if the BEP721 approval belongs to the owner

        if (IBEP721(BEP721Token).getApproved(tokenId) != address(this)) {
            revert("Not approved");
        }
        ...
        // @notice Transfer Non Fungible tokens (BEP721) from sender's account to target account.

        IBEP721(BEP721Token).transferFrom(msg.sender, target, tokenId);
